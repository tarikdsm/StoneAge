...
export async function publishMapSlotFile(file: MapSlotFile, token: string): Promise<MapSlotFile> {
  const result = validateMapSlotFileData(file)
  if (!result.value) {
    throw new Error(result.errors[0] ?? 'Map file is invalid.')
  }
...
export async function publishEditableLevel(editable: EditableLevelData, token: string): Promise<MapSlotFile> {
  return publishMapSlotFile(buildMapSlotFileFromEditableLevel(editable), token)
}

export async function clearMapSlot(slot: number, token: string): Promise<MapSlotFile> {
  const safeSlot = sanitizeSlot(slot)
  if (safeSlot === 1) {
    throw new Error('Map 01 can be modified, but never cleared.')
  }

  return publishMapSlotFile(createEmptyMapSlotFile(safeSlot), token)
}
